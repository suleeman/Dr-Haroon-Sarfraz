...
export const PatientPortal = ({ ...props }) => {
      const location = useLocation();
      const navigate = useNavigate();
      const {verifyUser} = location.state || {};
      const loggedinVerify = verifyUser ? true : false

    const [loggedIn, setLoggedIn] = useState(loggedinVerify);

  useEffect(() => {
    if (!loggedIn) {
      navigate("/PatientAuthenticationPortal");
    } 
  }, [loggedIn, navigate]);

    

  console.log(loggedIn,verifyUser);
              ...
              {Reports ? (
                <div className="appointment-details">
                  ...
                  >
                    ...
                    <div className="date">
                      {" "}
                      <p>Date: 25/1/25 </p> <p>v</p>
                    </div>{" "}
                    <div className="treatmentType">
                      {" "}
                      Treatment Type: Scaling and Cleaning
                    </div>{" "}
                  </div>
